...
const ViewReviews = () => {
  ...
  // State for user data of anonymous reviews
  const [anonymousUsers, setAnonymousUsers] = useState({});

  // Redux state
  const { hotel, reviews, loading, error } = useSelector(
    (state) => state.reviews,
  );
  ...
  // Fetch user data for anonymous reviews
  const fetchAnonymousUsers = async (reviews) => {
    const anonymousReviews = reviews.filter(review => review.userName === 'Anonymous User');
    const userIds = [...new Set(anonymousReviews.map(review => review.userId))];
  ...
  // Debug logging for reviews state
  useEffect(() => {
    if (reviews && reviews.length > 0) {
      // Fetch user data for anonymous reviews
      fetchAnonymousUsers(reviews);
    }
  }, [hotel, reviews, loading, error]);
  ...
  // Get current user info (for other purposes if needed)
  const currentUserId = sessionStorage.getItem('id');

  const avgRating = hotel.rating || 0;
  const totalReviews = hotel.totalReviews || reviews.length;

  // Process reviews to map anonymous reviews to actual user data
  const processedReviews = reviews.map(review => {
    if (review.userName === 'Anonymous User') {
      const userData = anonymousUsers[review.userId];
      if (userData) {
        return {
          ...review,
          userName: userData.name || review.userName,
          userEmail: userData.email || review.userEmail
        };
      }
    }
    return review;
                ...
                >
                  <Box>
                    ...
                    <Typography variant="body2" sx={{ mt: 1 }}>
                      {getRatingText(avgRating)} • {totalReviews} review
                      {totalReviews !== 1 ? "s" : ""}
                    </Typography>
                  </Box>
                      ...
                      <Box>
                        <Box display="flex" alignItems="center" gap={1}>
                          <Typography variant="subtitle1" fontWeight="bold">
                            {review.userName || review.userEmail || "Anonymous User"}
                          </Typography>
                          {review.verified && (
                            <Chip
                              icon={<Verified />}
